...
def traj_affordance_dist(hand_traj, contact_point, future_valid=None, invalid_value=9):
    ...
    contact_point = (
        contact_point.unsqueeze(dim=1).expand(-1, expand_size, 2).reshape(-1, 2)
    )  # (B * 2 * Tf, 2)
    dist = torch.sum((hand_traj - contact_point) ** 2, dim=1).reshape(
        batch_size, -1
    )  # (B, 2 * Tf)
    if future_valid is None:
        sorted_dist, sorted_idx = torch.sort(
        ...
        selected_dist, selected_idx = selected_dist.min(
            dim=1
        )  # selected_dist, selected_idx (B, )
        valid = torch.gather(
            future_valid, dim=1, index=selected_idx.unsqueeze(dim=1)
        ).squeeze(dim=1)
